...
#include "../include/adxl345_I2C.h"



using namespace std;

ADXL345_I2C::ADXL345_I2C(I2CDEV *i2c_ptr){
    i2c = i2c_ptr ;

	this->rawAccelX = 0;
	this->rawAccelY = 0;
	this->rawAccelZ = 0;
	this->range = PLUSMINUS_4_G;
...
int32_t ADXL345_I2C::writeReg(uint8_t regAddr, uint8_t value){
	uint8_t send[2] = {0} ;
	send[0] = regAddr ;
	send[1] = value ;
...
uint8_t ADXL345_I2C::readReg(uint8_t regAddr){
...
short ADXL345_I2C::combine_MSB_LSB(uint8_t msByte, uint8_t lsByte){
	//
   return ((short)msByte << 8) | (short)lsByte ;
}

  //讀取ADLX345 的暫存器資料
int32_t ADXL345_I2C::getRawAccel(){
	readAllRegs();
	if(regs_data[0] != 0xe5){
...
void ADXL345_I2C::calculate_AccelXYZ_g(){
...
void ADXL345_I2C::print_AccelXYZ(int32_t count){
